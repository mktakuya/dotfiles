# Jira チケットの description を更新する

Jira チケットの description（説明欄）を更新するワークフロー。

## 引数

- $ARGUMENTS: 更新対象のチケットキー（例: `PRT-388`）と、更新内容の指示
...
```bash
acli jira workitem view <TICKET_KEY> --fields description --json | jq '.fields.description' > /tmp/jira_desc_original.json
```
...
- `/tmp/jira_desc_original.json` を読み込み、指示された箇所を変更する
- ADF の構造:
...
編集後の ADF JSON を以下の形式でラップする:

```json
{
  "issues": ["<TICKET_KEY>"],
  "description": { ...編集後のADF JSON... }
}
```

ファイルに保存: `/tmp/jira_edit_payload.json`

### 4. チケットを更新

```bash
acli jira workitem edit --from-json /tmp/jira_edit_payload.json --yes
```
